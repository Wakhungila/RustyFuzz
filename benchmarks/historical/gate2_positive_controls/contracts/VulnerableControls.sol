...
contract OwnerMintToken is BurnOnlyToken {
    ...
    function mint(address to, uint256 amount) external {
        // Deliberate defect: owner authorization removed.
...
contract OffsetVault {
    ...
    function deposit(uint256 assets, address receiver) external returns (uint256 shares) {
        shares = convertToShares(assets); // Deliberate defect: zero-share deposits accepted.
...
contract NoFlashPool {
    ...
    function swap(uint256 out0, uint256 out1, address to, bytes calldata data) external {
        ...
        // Deliberate defect: reserve-product check removed.
...
contract CollateralizedLender {
    ...
    function borrow(uint256 amount) external {
        ...
        // Deliberate defect: borrowed assets are not recorded as debt.
...
contract GuardedGovernor {
    ...
    function execute(uint256 id) external {
        // Deliberate defect: proposal, quorum and timelock checks removed.
        require(id != 0);
        executed = true;
    }
}
