...
impl GlobalAccountRegistry {
    ...
    pub fn auto_populate_abi(&self, registry: &mut AbiRegistry) {
        let common_sigs: BTreeMap<[u8; 4], Vec<DynSolType>> = [
            ([0xa9, 0x05, 0x9c, 0xbb], vec![DynSolType::Address, DynSolType::Uint(256)]), // ERC20 transfer
            ([0x23, 0xb8, 0x72, 0xdd], vec![DynSolType::Address, DynSolType::Address, DynSolType::Uint(256)]), // ERC20 transferFrom
            ([0x09, 0x5e, 0xa7, 0xb3], vec![DynSolType::Address, DynSolType::Uint(256)]), // ERC20 approve
            ([0x81, 0x19, 0xc0, 0x65], vec![DynSolType::Address, DynSolType::Uint(256), DynSolType::Bytes]), // Proxy initialize
            ([0xb6, 0xb5, 0x5f, 0x25], vec![DynSolType::Uint(256), DynSolType::Address]), // Vault deposit
            ...
            ([0x61, 0x7c, 0x03, 0xcb], vec![DynSolType::Address, DynSolType::Uint(256), DynSolType::Uint(256), DynSolType::Uint(16)]), // Aave V3 supply
            ([0xa4, 0x15, 0xbb, 0x22], vec![DynSolType::Address, DynSolType::Uint(256), DynSolType::Uint(256), DynSolType::Address]), // Uniswap V3 swap
    ...
    pub async fn fetch_and_populate_abi(&self, address: Address, abi_registry: &mut AbiRegistry) -> anyhow::Result<()> {
    ...
    pub fn get_downstream_targets(&self, contract: &Address) -> Vec<Address> {
        self.call_graph.get(contract).map(|set| set.iter().cloned().collect()).unwrap_or_default()
    }
