...
impl Erc20Discovery {
    ...
    pub async fn find_balance_slot(
        ...
        let actual_balance = if let Ok(_gas) = self.executor.execute(
            &mut temp_state,
            &mut dummy_block_env,
            &tx,
        ...
        if actual_balance.is_zero() {
            return None;
        } // Cannot verify if balance is zero
            ...
            if let Some(token_acc) = initial_db.cache.accounts.get(&token_address) {
                if let Some(stored_balance) = token_acc.storage.get(&derived_slot) {
                    if *stored_balance == actual_balance {
                        log::info!(
                            "Discovered balance slot for Token {} at {}",
                            token_address,
                            derived_slot
                        );
    ...
    pub async fn find_total_supply_slot(
        &self,
        token_address: Address,
        initial_db: &EvmCacheDb,
    ) -> Option<U256> {
        ...
        if let Some(token_acc) = initial_db.cache.accounts.get(&token_address) {
            if token_acc.storage.contains_key(&U256::ZERO) {
                return Some(U256::ZERO);
            }
            if token_acc.storage.contains_key(&U256::from(1)) {
