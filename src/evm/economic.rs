...
impl EconomicState {
    ...
    pub fn calculate_profit(&self, attacker: Address, initial_state: &EconomicState) -> ProfitReport {
        let mut report = ProfitReport::default();

        // Native ETH profit
        let current_eth = self.native_balances.get(&attacker).copied().unwrap_or(U256::ZERO);
        let initial_eth = initial_state.native_balances.get(&attacker).copied().unwrap_or(U256::ZERO);
        report.eth_profit = current_eth.saturating_sub(initial_eth);

        // Token profits
        for (token, current_balances) in &self.balances {
            let current = current_balances.get(&attacker).copied().unwrap_or(U256::ZERO);
            let initial = initial_state.balances
                .get(token)
...
impl PriceAnalyzer {
    pub fn new() -> Self {
        Self { initial_prices: HashMap::new() }
    }
    ...
    pub fn check_manipulation(&self, pair: Address, current_price: U256, threshold_pct: u64) -> bool {
        if let Some(&initial) = self.initial_prices.get(&pair) {
            if initial == U256::ZERO { return false; }
