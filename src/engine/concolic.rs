...
impl<'a> ConcolicSolver<'a> {
    ...
    pub fn solve_hint(&self, waypoint: &Waypoint) -> Option<[u8; 32]> {
        match waypoint {
            Waypoint::Comparison { op, lhs: _, rhs, taint_source: Some(taint_source), .. } => {
            ...
            Waypoint::Arithmetic { op, lhs: _, rhs, third, taint_source: Some(taint_source), .. } => {
                let solver = Solver::new(self.ctx);
                let input_var = self.create_symbolic_var(taint_source)?;
                let rhs_bv = self.u256_to_bv(rhs)?;
                ...
                let constraint = match *op {
                    ...
                    0x08 | 0x09 => { // ADDMOD / MULMOD
                        if let Some(n) = third {
                            let n_bv = self.u256_to_bv(n)?;
                            let res = if *op == 0x08 { input_var.bvadd(&rhs_bv) } else { input_var.bvmul(&rhs_bv) };
    ...
    fn create_symbolic_var(&self, taint_source: &TaintSource) -> Option<BV<'a>> {
        match taint_source {
            TaintSource::Calldata(offset) => {
                if *offset >= 1024 * 1024 { return None; }
                Some(BV::new_const(self.ctx, format!("calldata_{}", offset), 256))
            }
            TaintSource::Storage(tx_idx, offset) => {
                Some(BV::new_const(self.ctx, format!("tx{}_storage_at_{}", tx_idx, offset), 256))
            }
        }
    }
