...
pub async fn run_fuzz_campaign(config: Config) -> anyhow::Result<()> {
    ...
    if !bytecode_selectors.is_empty() {
        ...
        log::debug!(
            "Bytecode selector evidence: {}",
            format_selector_list(&bytecode_selectors, 64)
        );
    }
    let launcher_fallback_bytecode_selectors = bytecode_selectors.clone();
    ...
    let launcher_result = Launcher::builder()
        ...
        .run_client(
            |state: Option<StdState<_, _, _, _>>, mut manager, description: ClientDescription| {
                log::info!(
                    "Broker client {:?} startup: begin client initialization",
                    description.core_id()
                );
                ...
                log::info!(
                    "Broker client {:?} startup: target selected {}",
                    description.core_id(),
                    target_contract
                );
                ...
                let mut startup_inserted_seed_count = 0usize;
                ...
                if state.corpus().count() == 0 {
                    ...
                    {
                        if let Some(profile) = target_profile.as_ref() {
                            if profile.confidence >= 35
                                && profile.protocol_types != vec![ProtocolType::Unknown]
                            {
                                let mut template_inputs = generate_flow_template_inputs(
                                    target_contract,
                                    fuzzer_address,
                    ...
                    startup_inserted_seed_count = inserted_seed_count;
                }

                let state_corpus_count = state.corpus().count();
                log::info!(
                    "Broker client {:?} corpus startup complete: inserted_seed_count={}, state_corpus_count={}",
                    core_id,
                    startup_inserted_seed_count,
                    state_corpus_count
                );

                if startup_inserted_seed_count > 0 && state_corpus_count == 0 {
                    return Err(libafl::Error::unknown(format!(
                        "broker client {:?} internal corpus wiring error: inserted_seed_count={} but state_corpus_count=0",
                        core_id,
                        startup_inserted_seed_count
                    )));
                }

                if state_corpus_count == 0 {
                    return Err(libafl::Error::unknown(format!(
                        "broker client {:?} cannot start fuzzing with empty LibAFL corpus",
                        core_id
                    )));
                }

                log::info!(
                    "Broker client {:?} creating scheduler, stages, fuzzer, and executor",
                    core_id
                );
                ...
                let first_harness_execution_logged = AtomicBool::new(false);

                let mut harness = |input: &EvmInput| {
                    if !first_harness_execution_logged.swap(true, Ordering::Relaxed) {
                        log::info!(
                            "Harness first execution observed: core={}, tx_count={}, base_snapshot_id={}",
                            core_id.0,
                            input.txs.len(),
                        ...
                        (config.target_invariant_manifest.as_deref(), economic_delta.as_ref())
                    {
                        match TargetInvariantManifest::load(path) {
                            Ok(manifest) => findings.extend(manifest.evaluate(delta)),
                            Err(err) => {
                                log::warn!("Failed to load target invariant manifest: {err:#}")
                            }
                        }
                    }
                    ...
                    if let Some(economic_delta) = economic_delta {
                        ...
                        if delta_score > 0 {
                            campaign_score.economic_pressure = campaign_score
                                ...
                                .saturating_add(delta_score);
                            campaign_score.total =
                                campaign_score.total.saturating_add(delta_score).min(10_000);
                            campaign_score.explanation.push(format!(
                                "hardened_defi_economic_delta: score={}, confidence={}, suspicious_extraction={}, accounting_anomaly={}",
                                delta_score,
                                economic_delta.confidence,
                    ...
                    if config.hardened_defi.enabled {
                        ...
                        let counterexample_pressure =
                            search_result.model.counterexample_pressure();
                        if counterexample_pressure > 0 {
                            campaign_score.counterexample_pressure = campaign_score
                                .counterexample_pressure
                                .saturating_add(counterexample_pressure);
                ...
                log::info!(
                    "Broker client {:?} creating InProcessExecutor with timeout {:?}",
                    core_id,
                    execution_timeout
                );
                ...
                log::info!(
                    "Broker client {:?} entering explicit fuzz_one loop: state_corpus_count={}",
                    core_id,
                    state.corpus().count()
                );

                let mut local_execs: u64 = 0;
                loop {
                    let _ = fuzzer.fuzz_one(&mut stages, &mut executor, &mut state, &mut manager)?;
                    local_execs += 1;

                    if local_execs == 1 || local_execs % 10_000 == 0 {
                        log::info!(
                            "Broker client {:?} fuzz_one progress: local_execs={}, state_corpus_count={}",
                            core_id,
                            local_execs,
                            state.corpus().count()
                        );
...
async fn run_single_process_campaign(
    ...
    log::info!(
        "Single-process startup: target selected {}",
        target_contract
    );
    ...
    let state_corpus_count = state.corpus().count();
    log::info!(
        "Single-process corpus startup complete: inserted_seed_count={}, state_corpus_count={}",
        startup_inserted_seed_count,
        state_corpus_count
    );
    ...
    if startup_inserted_seed_count > 0 && state_corpus_count == 0 {
        anyhow::bail!(
            "single-process internal corpus wiring error: inserted_seed_count={} but state_corpus_count=0",
            startup_inserted_seed_count
        );
    }

    if state_corpus_count == 0 {
        anyhow::bail!("single-process campaign cannot start with empty LibAFL corpus");
    }

    log::info!("Single-process creating scheduler, stages, fuzzer, and executor");
    ...
    log::info!(
        "Single-process creating InProcessExecutor with timeout {:?}",
        execution_timeout
    );
    ...
    log::info!(
        "Single-process entering explicit fuzz_one loop: state_corpus_count={}",
        state.corpus().count()
    );
    ...
    loop {
        ...
        if local_execs == 1 || local_execs % 10_000 == 0 {
            log::info!(
                "Single-process fuzz_one progress: local_execs={}, state_corpus_count={}",
                local_execs,
                state.corpus().count()
            );
        }
...
fn selector_to_hex(selector: &[u8; 4]) -> String {
    format!(
        "0x{:02x}{:02x}{:02x}{:02x}",
...
fn format_selector_list(selectors: &[[u8; 4]], limit: usize) -> String {
    ...
    if selectors.len() > limit {
        rendered.push(format!("...(+{} more)", selectors.len() - limit));
    }
...
mod tests {
    ...
    #[test]
    fn selector_formatting_is_hex_encoded_and_bounded() {
        let selectors = vec![[0x12, 0x34, 0xab, 0xcd], [0xde, 0xad, 0xbe, 0xef]];
        assert_eq!(selector_to_hex(&selectors[0]), "0x1234abcd");
        assert_eq!(
            format_selector_list(&selectors, 1),
            "0x1234abcd,...(+1 more)"
        );
    }
}
