    ...
    pub fn record_flow(&mut self, source: TaintSource, sink: TaintSink, path: Vec<TaintOperation>) {
        ...
        let description = match &sink {
            TaintSink::DelegateCallTarget => {
                "Tainted data controls DELEGATECALL target (code execution)".to_string()
            }
            ...
            TaintSink::AccessControlCheck => {
                "Tainted data used in access control comparison".to_string()
            }
            ...
            TaintSink::Create2Salt => {
                "Tainted data controls CREATE2 address prediction".to_string()
            }
...
impl<'a> TaintInspector<'a> {
    ...
    fn propagate_binary(&mut self, op: TaintOperation, stack: &Stack) {
        ...
        let top_state = self
            .tracker
            .get_stack_taint(stack_len - 1)
            .cloned()
            .unwrap_or_default();
        let second_state = self
            .tracker
            .get_stack_taint(stack_len - 2)
            .cloned()
            .unwrap_or_default();
